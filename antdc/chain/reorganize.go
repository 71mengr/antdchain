...
type ChainFork struct {
	...
	ActiveChain    []*block.Block `json:"-"`              // Active chain blocks (oldest first)
	CandidateChain []*block.Block `json:"-"`              // Candidate chain blocks (oldest first)
...
func (rm *ReorgManager) performReorg(fork *ChainFork) error {
	...
	// ActiveChain is stored oldest-to-newest, so keep track of exactly which
	// blocks were detached.  If a disconnect fails before a block is detached,
	// reconnecting the full active chain would try to insert blocks that are
	// still canonical and can cause duplicate-block rollback errors.
	disconnected := make([]*block.Block, 0, len(fork.ActiveChain))
	for i := len(fork.ActiveChain) - 1; i >= 0; i-- {
		...
		if err := rm.chain.DisconnectBlock(height); err != nil {
			...
			// Attempt to reconnect only blocks that were actually disconnected.
			rm.emergencyRollback(disconnected)
	...
	for i, blk := range fork.CandidateChain {
		...
		if err := rm.chain.ConnectBlock(blk); err != nil {
			...
			// Attempt to reconnect the original chain after removing any candidate
			// blocks that were connected before this failure.
			rm.emergencyRollbackFrom(fork, i, disconnected)
...
// emergencyRollback rolls back a failed reorg attempt by restoring the blocks
// that were already disconnected. The input slice is collected newest-to-oldest,
// so reconnect it oldest-to-newest to preserve parent/child ordering.
func (rm *ReorgManager) emergencyRollback(disconnected []*block.Block) {
...
// emergencyRollbackFrom rolls back from a specific candidate connection point.
func (rm *ReorgManager) emergencyRollbackFrom(fork *ChainFork, failedCandidateIndex int, disconnected []*block.Block) {
	rm.logger.Errorf("EMERGENCY ROLLBACK from candidate index %d", failedCandidateIndex)

	// Disconnect candidate blocks that were successfully connected before the
	// failed candidate. The failed block itself may not have been connected.
	for i := failedCandidateIndex - 1; i >= 0; i-- {
